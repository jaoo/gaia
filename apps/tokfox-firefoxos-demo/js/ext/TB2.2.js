...
/**
* This base class defines the <code>on</code>, <code>once</code>, and <code>off</code> methods of objects
...
OTHelpers.eventing = function(self, syncronous) {
  ...
  * </pre>
  *
  * <p>If you pass in multiple space-separated event names and a handler method, the handler is 
  ...
  * </pre>
  *
  * <p>You can also pass in a third <code>context</code> parameter (which is optional) to define the value of
  ...
  * that has event names as its keys and corresponding handler functions as values. The second 
  * parameter (optional) is the context for <code>this</code> in each handler:
  * </p>
  * <pre>
  * obj.on(
  *   {
  ...
  * </p>
  *
  * @param {String} type The string identifying the type of event. You can specify multiple event names in this string,
  * separating them with a space. The event handler will process each of the events.
  * @param {Function} handler The handler function to process the event. This function takes the event object as a parameter.
  * @param {Object} context (Optional) Defines the value of <code>this</code> in the event handler function.
  *
  * @returns {Object} The object on which the method was called. This lets you chain methods,
  * as in the following:
  * 
  * <pre>
  ...
  *
  * <p>If you pass in one event name and a handler method, the handler is removed for that event:</p>
  *
  * <pre>obj.off("eventName", eventHandler);</pre>
  *
  * <p>If you pass in multiple space-separated event names and a handler method, the handler
  * is removed for those events:</p>
  *
  * <pre>obj.off("eventName1 eventName2", eventHandler);</pre>
  *
  * <p>If you pass in an event name (or names) and <i>no</i> handler method, all handlers are removed for those events:</p>
  *
  * <pre>obj.off("event1Name event2Name");</pre>
  *
  * <p>If you pass in no arguments, <i>all</i> event handlers are removed for all events dispatched by the object:</p>
  *
  * <pre>obj.off();</pre>
  *
  * <p>
  ...
  * </pre>
  *
  * @param {String} type (Optional) The string identifying the type of event. You can use a space to specify
  * multiple events, as in "accessAllowed accessDenied accessDialogClosed". If you pass in no <code>type</code> value
  * (or other arguments), all event handlers are removed for the object.
  * @param {Function} handler (Optional) The event handler function to remove. The handler must be the same function object
  * as was passed into <code>on()</code>. Be careful with helpers like <code>bind()</code> that return a new function when called.
  * If you pass in no <code>handler</code>, all event handlers are removed for the specified event <code>type</code>.
  * @param {Object} context (Optional) If you specify a <code>context</code>, the event handler is removed for
  * all specified events and handlers that use the specified context. (The context must match the context passed into
  * <code>on()</code>.)
  *
  * @returns {Object} The object on which the method was called.
  *
  * @memberOf EventDispatcher
  * @method #off
  * @see <a href="#on">on()</a>
 ...
 /**
  * Adds an event handler function for one or more events. Once the handler is called, the specified handler method is
  * removed as a handler for this event. (When you use the <code>on()</code> method to add an event handler, the handler
  * is <i>not</i> removed when it is called.) The <code>once()</code> method is the equivilent of calling the <code>on()</code>
  * method and calling <code>off()</code> the first time the handler is invoked.
  *
  * <p>
  * The following code adds a one-time event handler for an event:
  * </p>
  *
  * <pre>
  ...
  *
  * <p>If you pass in multiple space-separated event names and a handler method, the handler
  * is registered for each of those events:</p>
  *
  * <pre>
  ...
  *     },
  *     eventName2:  function (event) {
  *       // This is the event handler for eventName2.
  *     }
  *   },
  *   obj
  * );
  ...
  *
  * @param {String} type The string identifying the type of event. You can specify multiple event names in this string,
  * separating them with a space. The event handler will process the first occurence of the events. After the first event,
...
!(function(window) {
  ...
  * <p>
  * Calling <code>TB.setLogLevel()</code> sets the log level for runtime log messages that
  * are the OpenTok library generates. The default value for the log level is <code>TB.ERROR</code>.
  * </p>
  * <p>
  ...
  * </p>
  * <p>
  * The following example logs the session ID to the console, by calling <code>TB.log()</code>.
  ...
  *   </li>
  *   <li>
  *     <code>TB.INFO</code> &#151; Logging of other useful information, in addition to
  *     warnings and errors.
  *   </li>
  *   <li>
  *     <code>TB.LOG</code> &#151; Logging of <code>TB.log()</code> messages, in addition
  *     to OpenTok info, warning,
  *     and error messages.
  *   </li>
  *   <li>
  *     <code>TB.DEBUG</code> &#151; Fine-grained logging of all API actions, as well as
  *     <code>TB.log()</code> messages.
  *   </li>
  * </ul>
  * </p>
  *
  ...
  * to <code>TB.LOG</code> or <code>TB.DEBUG</code>,
...
!(function(window) {
  ...
  gumNamesToMessages = {
    PERMISSION_DENIED: 'User denied permission for scripts from this origin to ' +
...
// Map vendor error strings to names and messages
  ...
  OT.$.supportsWebRTC = function() {
    ...
    if (navigator.webkitGetUserMedia) {
      ...
      var firefoxVersion = window.navigator.userAgent.toLowerCase()
          .match(/Firefox\/([0-9\.]+)/i);
      _supportsWebRTC = typeof(mozRTCPeerConnection) === 'function' &&
        (firefoxVersion !== null && parseFloat(firefoxVersion[1], 10) > 20.0);
      if (_supportsWebRTC) {
        try {
  ...
  OT.$.getUserMedia = function(constraints, success, failure, accessDialogOpened,
    accessDialogClosed, accessDenied) {
    ...
    var triggerOpenedTimer = null,
        ...
        onError = function(event) {
          ...
          if (error.name === 'PermissionDeniedError' || error.name === 'PERMISSION_DENIED') {
      ...
      isRotated: {
        get: function() {
          return this._orientation && (
            this._orientation.videoOrientation === 'OTVideoOrientationRotatedLeft' ||
            this._orientation.videoOrientation === 'OTVideoOrientationRotatedRight'
          );
        }
      },
...
/**
* The first step in using the OpenTok API is to call the <code>TB.initSession()</code>
* method. Other methods of the TB object check for system requirements and set up error logging.
*
* @class TB
*/
...
*   <code>TB.initPublisher()</code> to publish to different sessions sequentially,
...
*   <code>TB.initPublisher()</code> method, if the browser does not support the requested
*   resolution.
*   </p>
*   <p>
*   For OpenTok cloud-enabled sessions, lowering the frame rate or lowering the resolution reduces
...
*   includes one property: <code>nameDisplayMode</code>. Possible values for the
*   <code>style.nameDisplayMode</code> property are: <code>"auto"</code> (the name is displayed
*   when the stream is first displayed and when the user mouses over the display),
*   <code>"off"</code> (the name is not displayed), and <code>"on"</code> (the name is
    displayed).</li>
* </li>
* <li>
...
* <code>TB.initPublisher()</code> method:
* </p>
* <pre>
* var publisher = TB.initPublisher(apiKey, 'publisher', null, function (error) {
*   if (error) {
*     console.log(error);
*   } else {
...
* <code>TB.initSession()</code> or the <code>TB.initPublisher()</code> method
  ...
  OT.upgradeSystemRequirements = function(){
    ...
    OT.onLoad( function() {
      ...
      document.body.appendChild((function() {
        ...
        d.src = OT.properties.assetURL + '/html/upgradeFlash.html#' +
...
* 
* <p>
* Registers a method as an event listener for a specific event. Note that this is a static method of
* the TB class.
* </p>
*
* <p>
* The TB object dispatches one type of event &#151; an <code>exception</code> event. The
...
/**
* Adds an event handler function for one or more events. Note that this is a static method of the TB
* class.
*
* <p>
* The TB object dispatches one type of event &#151; an <code>exception</code> event. The following 
* code adds an event
...
* method is
* removed as a handler for this event. (When you use the <code>TB.on()</code> method to add an event
* handler, the handler
* is <i>not</i> removed when it is called.) The <code>TB.once()</code> method is the equivilent of 
* calling the <code>TB.on()</code>
* method and calling <code>TB.off()</code> the first time the handler is invoked.
*
* <p>
* Note that this is a static method of the TB class.
* </p>
*
* <p>
...
*
* <pre>
* TB.once("exception", function (event) {
*   console.log(event);
* }
* </pre>
*
 ...
 * <code>TB.on()</code> method, which is a <i>static</i> method.
 *
 * @name exception
 * @event
...
!(function() {
  ...
  * <code>TB.on()</code> method, which is a <i>static</i> method.</p>
  *
...
!(function() {
  ...
  OT.Raptor.Socket = function(widgetId, messagingSocketUrl, symphonyUrl, Dispatcher) {
    ...
    this.streamCreate = function(name, orientation, encodedWidth, encodedHeight,
      hasAudio, hasVideo, frameRate, completion) {
      var streamId = OT.$.uuid(),
          message = OT.Raptor.Message.streams.create( OT.APIKEY,
                                                      ...
                                                      hasVideo,
                                                      frameRate,
                                                      TB.Config.get('bitrates', 'min', OT.APIKEY),
                                                      TB.Config.get('bitrates', 'max', OT.APIKEY));
    ...
    OT.$.defineGetters(this, {
      id: function() { return _rumor && _rumor.id; },
      sessionId: function() { return _sessionId; }
    });

    if(Dispatcher == null) {
      Dispatcher = OT.Raptor.Dispatcher;
    }
...
!(function() {
  ...
  function getSession (message) {
    var session = OT.sessions.get(message.params.session);

    if (!session) {
      OT.error('OT.Raptor.dispatch: Unable to determine sessionId, or the session does ' +
        'not exist, for ' + message.signature + ' message!');
      // @todo error
    }
  ...
  OT.Raptor.Dispatcher = function (socket) {
    this.socket = socket;
    this.callbacks = {};
  };
  ...
  OT.Raptor.Dispatcher.prototype.dispatchSession = function (message) {
    switch (message.method) {
      case 'read':
        ...
        var state = {},
            session = OT.sessions.get(message.params.session),
            connection;
        ...
        message.content.connection.forEach(function(connectionParams) {
          connection = OT.Connection.fromHash(connectionParams);
          state.connections.push(connection);
          session.connections.add(connection);
        });

        message.content.stream.forEach(function(streamParams) {
          state.streams.push( parseAndAddStreamToSession(streamParams, session) );
        });
        
        message.content.archives.forEach(function(archiveParams) {
          state.archives.push( parseAndAddArchiveToSession(archiveParams, session) );
        });

        session._.subscriberMap = {};

        this.triggerCallback(message.transactionId, null, state);
        break;
  ...
  OT.Raptor.Dispatcher.prototype.dispatchConnection = function (message) {
    ...
    switch (message.method) {
      case 'created':
        connection = OT.Connection.fromHash(message.content);
      ...
      case 'deleted':
        connection = session.connections.get(message.params.connection);
        connection.destroy(message.reason);
        break;
  ...
  OT.Raptor.Dispatcher.prototype.dispatchStream = function (message) {
    ...
    switch (message.method) {
      case 'created':
        stream = parseAndAddStreamToSession(message.content, session);

        if (stream.publisher) {
          stream.publisher.stream = stream;
        }

        this.triggerCallback(message.transactionId, null, stream);
        break;


      case 'deleted':
        stream = session.streams.get(message.params.stream);

        if (!stream) {
          OT.error('OT.Raptor.dispatch: A stream does not exist with the id of ' +
            message.params.stream + ', for ' + message.signature + ' message!');
          // @todo error
          return;
        }

        stream.destroy(message.reason);
        break;
      ...
      case 'updated':
        ...
        if (!stream || stream.destroyed) {
          OT.error('OT.Raptor.dispatch: A stream ' +
            (!stream ? 'does not exist' : 'has been destroyed') + ' with the id of ' +
            message.params.stream + ', for ' + message.signature + ' message!');
          // @todo error
          return;
        }

        stream._.update(message.content);
        break;
  ...
  OT.Raptor.Dispatcher.prototype.dispatchStreamChannel = function (message) {
    ...
    if (!(message.params.stream && (stream = session.streams.get(message.params.stream)))) {
      OT.error('OT.Raptor.dispatch: Unable to determine streamId, or the stream does not exist, ' +
        'for ' + message.signature + ' message!');
      // @todo error
      return;
    }
    ...
    switch (message.method) {
      case 'updated':
        stream._.updateChannel(message.params.channel, message.content);
        break;
  ...
  OT.Raptor.Dispatcher.prototype.dispatchJsep = function (method, message) {
    var fromConnection,
        streamId = message.params.stream,
        actors;
    ...
    fromConnection = actors[0].session.connections.get(message.fromAddress);
    if(!fromConnection && message.fromAddress.match(/^symphony\./)) {
      fromConnection = OT.Connection.fromHash({
        id: message.fromAddress,
      ...
      OT.warn('OT.Raptor.dispatch: Messsage comes from a connection (' +
        message.fromAddress + ') that we do not know about. The message was ignored.');
      return;
    }
  ...
  OT.Raptor.Dispatcher.prototype.dispatchSubscriberChannel = function (message) {
    ...
    if (!message.params.stream || !session.streams.has(message.params.stream)) {
      OT.error('OT.Raptor.dispatch: Unable to determine streamId, or the stream does not ' +
        'exist, for ' + message.signature + ' message!');
      // @todo error
      return;
    }
    ...
    switch (message.method) {
      case 'updated':
        session.streams.get(message.params.stream)._
          .updateChannel(message.params.channel, message.content);
        break;


      case 'update':
        // Hint to update for congestion control from the Media Server
        if (!OT.subscribers.has(message.params.subscriber)) {
          OT.error('OT.Raptor.dispatch: Unable to determine subscriberId, or the subscriber ' +
            'does not exist, for ' + message.signature + ' message!');
          // @todo error
          return;
        }
      ...
      // we may need to be more specific in the future
        OT.subscribers.get(message.params.subscriber).disableVideo(message.content.active);
        break;
  ...
  OT.Raptor.Dispatcher.prototype.dispatchSubscriber = function (message) {
    ...
    stream = message.params.stream ? session.streams.get(message.params.stream) : null;
    ...
    switch (message.method) {
      case 'created':
        session._.subscriberMap[message.fromAddress + '_' + stream.id] = message.content.id;
        break;


      case 'deleted':
        this.dispatchJsep('unsubscribe', message);
        delete session._.subscriberMap[message.fromAddress + '_' + stream.id];
        break;
  ...
  OT.Raptor.Dispatcher.prototype.dispatchSignal = function (message) {
    ...
    session._.dispatchSignal( session.connections.get(message.fromAddress),
  ...
  OT.Raptor.Dispatcher.prototype.dispatchArchive = function (message) {
    ...
    switch (message.method) {
      case 'create':
        parseAndAddArchiveToSession(message.content, session);
        break;
      
      case 'updated':
        archive = session.archives.get(message.params.archive);

        if (!archive) {
          OT.error('OT.Raptor.dispatch: An archive does not exist with the id of ' +
            message.params.archive + ', for ' + message.signature + ' message!');
          // @todo error
          return;
        }

        archive._.update(message.content);
        break;
    }
  };
...
//  TB.exceptionHandler('publisher_1234,1234',
 ...
 */
  ...
  OT.Stream = function(id, name, creationTime, connection, session, channel) {
    ...
    this.setChannelActiveState = function(channelType, activeState) {
      updateChannelsOfType(channelType, {
        active: activeState
      });
    };
          ...
          if (newState && newState.toLowerCase() !== _state) {
            ...
            switch(_state) {
              case 'closed':
                ...
                break;
              case 'failed':
                triggerError('Ice state failed', 'ICEWorkflow');
                break;
            }
          }
        },
 ...
 */
   ...
   *
   * <p>If you pass one parameter, <code>style</code>, it is an object that has one property: 
   * <code>nameDisplayMode</code>. Possible values for the <code>style.nameDisplayMode</code> 
   * property are: "auto" (the name is displayed when the stream is first displayed and when the 
   * user mouses over the display), "off" (the name is not displayed), and "on" (the name is 
   * displayed).
   *
   ...
   *
   * <p>The TB object dispatches an <code>exception</code> event if you pass in an invalid style 
   ...
   * <p>If you pass one parameter, <code>style</code>, it is an object that has one property: 
   * <code>nameDisplayMode</code>. Possible values for the <code>style.nameDisplayMode</code> 
   * property are: "auto" (the name is displayed when the stream is first displayed and when 
   * the user mouses over the display), "off" (the name is not displayed), and "on" (the name 
   * is displayed).
   *
   * <p>For example, the following code passes one parameter to the method:</p>
   *
 ...
 * published stream is accomplished. Calling the <code>TB.initPublisher()</code> method
 ...
 *      publisher = TB.initPublisher(API_KEY, publisherContainer, publisherProperties);
 *      session.publish(publisher);
 *  });
 ...
 *      named <code>publisherContainer</code> by calling the <code>TB.initPublisher()</code>
 ...
 * property of the <code>TB.initPublisher()</code> method, if the browser does not support
 ...
 * of the <code>TB.initPublisher()</code> method, if the browser does not support the requested
 * resolution.
 *
 * @see <a href="TB.html#initPublisher">TB.initPublisher</a>
  ...
  OT.Publisher = function() {
    ...
    var logAnalyticsEvent = function(action, variation, payloadType, payload) {
        ...
        onStreamAvailableError = function(error) {
          ...
          _state.set('Failed');
          this.trigger('publishComplete', new OT.Error(2000, error.message));
        ...
        onVideoError = function(errorCode, errorReason) {
          ...
          if (_state.attemptingToPublish) {
            this.trigger('publishComplete', new OT.Error(2000, message));
          } else {
        ...
        assignStream = function(stream) {
          ...
          this.dispatchEvent(new OT.StreamEvent('streamCreated', stream));
 ...
 */
  OT.Subscriber = function(targetElement, options) {
    ...
    * @memberOf Subscriber
    */
    this.subscribeToVideo = function(pValue) {
...
!(function() {
  ...
  OT.SessionInfo = function(xmlDocument) {
    ...
    if (xmlDocument && xmlDocument.documentElement &&
      xmlDocument.documentElement.firstElementChild !== null) {
  ...
  OT.SessionInfo.get = function(session, onSuccess, onFailure) {
    ...
    OT.$.getXML(sessionInfoURL, {
      ...
      error: function(event) {
        onGetErrorCallback(session, onFailure, event.target.responseXML ?
          parseErrorFromXMLDocument(event.target.responseXML) :
          new OT.Error(event.target && event.target.status, event.message ||
            'Could not connect to Tokbox API Server'));
      }
    });
  };
  ...
  parseErrorFromXMLDocument = function(xmlDocument) {
    if (xmlDocument && xmlDocument.documentElement &&
      xmlDocument.documentElement.firstElementChild !== null) {
      var errorNodes = xmlDocument.evaluate('//error',
          xmlDocument.documentElement, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null ),
  ...
  onGetErrorCallback = function(session, onFailure, error) {
    TB.handleJsException('TB.SessionInfoError :: Unable to get session info ' + error.message,
      messageServerToClientErrorCodes[error.code], {
      session: session
    });
  ...
  parseIceServersXml = function (nodes) {
    ...
    for (var i=0, numNodes=nodes.length; i<numNodes; ++i) {
      if (nodes[i].localName === 'ice_server') {
        ...
        if (attributes.getNamedItem('credential') &&
          attributes.getNamedItem('credential').nodeValue.length) {
...
/**
 * The Session object returned by the <code>TB.initSession()</code> method provides access to
 ...
 * <code>TB.initSession()</code> method when you create the Session object. (Note: a Session
 * object is not connected to the OpenTok server until you call the connect() method of the
 * object and the object dispatches a connected event. See {@link TB.initSession} and
 * {@link connect}).
 ...
 *  TB object dispatches an `exception` event (with the `code` property set to 1007). See
 ...
 *    by the TB class locally in the event of an error.
 *  </p>
 *  <p>
 ...
 *  </p>
 *
  * @param {String} apiKey The API key that TokBox provided you when you registered for the
  * OpenTok API.
  *
  ...
  */
    this.connect = function(apiKey, token) {
      ...
      if(apiKey === null || apiKey === undefined || OT.$.isFunction(apiKey)) {
        setTimeout(sessionConnectFailed.bind(this, 403, 'API Key is invalid'));
        return this;
      }
  ...
  * <p>
  * <i>Note:</i> If you intend to reuse a Publisher object created using
  * <code>TB.initPublisher()</code> to publish to different sessions sequentially, call either
  ...
  * Publisher object by calling the <a href="TB.html#initPublisher">TB.initPublisher()</a>
  ...
  * defines options for the Publisher (see <a href="TB.html#initPublisher">TB.initPublisher()</a>.)
  ...
  * <p>
  * <code>exception</code> (<a href="ExceptionEvent.html">ExceptionEvent</a>) &#151; Dispatched
  * by the TB object. This can occur when user's role does not allow publishing (the
  ...
  * <a href="TB.html#initPublisher">TB.initPublisher()</a> method.
  *
  * @param {Function} completionHandler (Optional) A function to be called when the call to the
  ...
  */
    this.publish = function(publisher, properties, completionHandler) {
      ...
      if (this.isNot('connected')) {
        /*jshint camelcase:false*/
        _analytics.logError(1010, 'tb.exception',
  ...
  * <p>
  * <i>Note:</i> If you intend to reuse a Publisher object created using
  * <code>TB.initPublisher()</code> to publish to different sessions sequentially, call
  ...
  *
  *       <li>
  *         <strong>insertMode</strong> (String) &#151; Specifies how the Subscriber object will
  ...
  *       </li>
  *
  *       <li><code>style.nameDisplayMode</code> (String) &#151; This property determines whether
  *       to display the stream name. Possible values are: <code>"auto"</code> (the name is
  *       displayed when the stream is first displayed and when the user mouses over the
  *       display), <code>"off"</code> (the name is not displayed),and <code>"on"</code>
  *       (the name is displayed).</li>
  *
  ...
  * <p>
  * Each property is optional. If you set none of the properties, you will send a signal with
  * no data or type to
  ...
  *   <li><code>data</code> &mdash; (Object) The data to send. Valid types for data are a
  *     JSON-parsable object, a JSON-parsable array, a string, a number, a Boolean value, or
  *     <code>null</code>. Numbers cannot be NaN or Infinity. The limit to the size of data
  *     is 8kB.</li>
  ...
  * arguments. On error, the function is passed an <code>error</code> object parameter. The
  * <code>error</code> object has the following properties:
  *
  * <ul>
  ...
  *   <a href="https://dashboard.tokbox.com/projects">Dashboard</a> page.
  *   See <a href="ExceptionEvent.html">ExceptionEvent</a> and <a href="TB.html#on">TB.on()</a>.
  * </p>
  * <p>
  *   The application throws an error if the session is not connected.
  * </p>
  ...
  * parameter. The <code>error</code> object has two properties: <code>code</code> (an integer)
  ...
  */
   ...
   * calling the <code>TB.initSession()</code> method. For a code example and more details,
